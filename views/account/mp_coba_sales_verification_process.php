<?php
/**
 * Account - MP COBA Sales Verification Process
...
if ($submission) {
    // Calculate Berhad-focused net amount: MP COBA Sales - MP COBA Expenses
    $berhadNetAmount = (float) $submission['mp_coba_sales'];
    ...
    // Recalculate Berhad-focused net amount after loading expenses
    $berhadNetAmount = (float) $submission['mp_coba_sales'] - $totalMpBerhadExpenses;
...
<head>
    ...
    <title>MP COBA Verification Process - <?php echo htmlspecialchars(APP_NAME); ?></title>
    <style>
        * {
        ...
        .comparison-feedback.loading::before {
            content: '';
            position: absolute;
            left: 16px;
            top: 50%;
        ...
        .loading-progress-bar {
            height: 100%;
            background: linear-gradient(90deg, #5b21b6 0%, #10b981 50%, #5b21b6 100%);
...
<body>
    <div class="header">
        <div class="breadcrumbs">
            ...
            <span>›</span>
            <a href="mp_coba_sales_verification.php">MP COBA Sales Verification</a>
            <span>›</span>
            <strong>Verification Process</strong>
        </div>
        <h1>MP COBA Sales Verification Process</h1>
        <p>Review the submission details for the selected outlet before confirming the MP COBA sales.</p>
    </div>
    ...
    <div class="container">
        <?php if (!empty($errors)) : ?>
            <div class="alert">
                ...
                <div class="actions" style="margin-top:16px;">
                    <a class="btn btn-secondary" href="mp_coba_sales_verification.php">Back to MP COBA Verification</a>
                </div>
            </div>
        <?php elseif ($submission) : ?>
            ...
            <div class="card">
                ...
                <div class="grid">
                    <div class="stat">
                        <div class="label">MP COBA Sales</div>
                        <div class="value">RM <?php echo number_format((float) $submission['mp_coba_sales'], 2); ?></div>
                    </div>
                    <div class="stat">
                        <div class="label">MP COBA CLAIMED</div>
                        <div class="value">RM <?php echo number_format($totalMpBerhadExpenses, 2); ?></div>
                    </div>
                    <div class="stat">
                        <div class="label">MP COBA Net Amount</div>
                        <div class="value">RM <?php echo number_format($berhadNetAmount, 2); ?></div>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <?php else : ?>
                    <div class="empty-state">
                        No MP MP COBA expenses have been linked to this submission yet.
                    </div>
                <?php endif; ?>
                ...
                <div class="comparison-input">
                    ...
                    <textarea
                        ...
                        data-manager-berhad-sales="<?php echo htmlspecialchars(number_format((float) ($submission['mp_coba_sales'] ?? 0), 2, '.', '')); ?>"
                        data-manager-expenses="<?php echo htmlspecialchars(number_format((float) $totalMpBerhadExpenses, 2, '.', '')); ?>"
                        data-manager-net-amount="<?php echo htmlspecialchars(number_format($berhadNetAmount, 2, '.', '')); ?>"><?php echo htmlspecialchars($externalSalesRawData); ?></textarea>
            ...
            <div class="actions">
                <a class="btn btn-primary" href="mp_coba_sales_verification.php">Return to Submission List</a>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function setComparisonFeedback(state, summary, details, showLoading) {
                ...
                if (showLoading && state === 'loading') {
                    ...
                    comparisonFeedback.appendChild(progressBar);

                    const steps = document.createElement('div');
                    steps.className = 'analysis-steps';
                    steps.innerHTML = `
                        <div class="step" id="step1">Parsing external sales data...</div>
                        <div class="step" id="step2">Calculating totals and deposits...</div>
                        <div class="step" id="step3">Comparing against manager submission...</div>
                        <div class="step" id="step4">Validating differences and generating report...</div>
                    `;
                    comparisonFeedback.appendChild(steps);

                    setTimeout(function() { document.getElementById('step1').classList.add('active'); }, 300);
                    setTimeout(function() {
            ...
            function handleCompare() {
                ...
                setComparisonFeedback('loading', 'Analyzing and comparing data...', null, true);

                window.setTimeout(function () {
                    const totals = computeExternalTotals(latestParsedRows);
                    const comparisons = [
                        {
                            label: 'MP COBA Sales vs Total Deposit',
                            managerValue: managerBerhadSales,
                            externalValue: totals.totalDeposit
                        },
                        {
                            label: 'MP MP COBA Expenses vs Total Withdraw',
