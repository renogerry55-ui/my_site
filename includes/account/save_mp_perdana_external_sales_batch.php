<?php
/**
 * Save MP PERDANA External Sales Data (Batch)
...
try {
    ...
    // Get all pending MP PERDANA submissions for this manager
    $submissions = dbFetchAll(
        ...
        WHERE ds.manager_id = :manager_id
          ...
          AND ds.mp_perdana_sales > 0
        ORDER BY o.outlet_name ASC",
        ['manager_id' => $managerId]
    );

    if (empty($submissions)) {
        throw new Exception('No pending MP PERDANA submissions found for this manager.');
    }
    ...
    foreach ($submissions as $submission) {
        ...
        $claimedMpPerdanaSales = (float) $submission['mp_perdana_sales'];

        // Delete existing external data for this submission
        dbQuery(
            "DELETE FROM mp_perdana_external_sales_data WHERE submission_id = :id",
            ['id' => $submissionId]
        );
        ...
        // Query expenses where category_name = 'MP PERDANA'
        ...
        if (empty($matchingRows)) {
            ...
            $comparisonResults[] = [
                ...
                'submitted_sales' => $claimedMpPerdanaSales,
                'external_sales' => 0,
                'sales_difference' => -$claimedMpPerdanaSales,
                'sales_matches' => false,
        ...
        foreach ($matchingRows as $rowData) {
            ...
            dbQuery(
                "INSERT INTO mp_perdana_external_sales_data (
        ...
        $salesDifference = $externalSalesTotal - $claimedMpPerdanaSales;
    ...
    error_log('MP PERDANA batch external sales save error: ' . $e->getMessage());
}
