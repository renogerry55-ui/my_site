<?php
/**
 * Save Berhad External Sales Data (Batch)
...
try {
    ...
    // Get all pending Berhad submissions for this manager
    ...
    if (empty($submissions)) {
        throw new Exception('No pending Berhad submissions found for this manager.');
    }
    ...
    foreach ($submissions as $submission) {
        ...
        $claimedBerhadSales = (float) $submission['berhad_sales'];

        // Delete existing external data for this submission
        dbQuery(
            "DELETE FROM berhad_external_sales_data WHERE submission_id = :id",
            ['id' => $submissionId]
        );
        ...
        // Query expenses where category_name = 'BERHAD'
        ...
        if (empty($matchingRows)) {
            ...
            $comparisonResults[] = [
                ...
                'submitted_sales' => $claimedBerhadSales,
                'external_sales' => 0,
                'sales_difference' => -$claimedBerhadSales,
                'sales_matches' => false,
        ...
        $salesDifference = $externalSalesTotal - $claimedBerhadSales;
    ...
    error_log('Batch external sales save error: ' . $e->getMessage());
}
